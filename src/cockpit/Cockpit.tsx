...
export function Cockpit({ brainOnline, xaiReady = false, museReady = false, onConnect }: CockpitProps) {
  ...
  const talkQuiet = talkOpen || speakDemo || mode === 'talk';
  const whoop = useWhoopDay(brainOnline, !talkQuiet);
  const calendar = useCalendarWeek(brainOnline, !talkQuiet);
  const locks = useType1Locks(brainOnline, !talkQuiet);
  const jobs = useAgentJobs(brainOnline, locks, !talkQuiet);
